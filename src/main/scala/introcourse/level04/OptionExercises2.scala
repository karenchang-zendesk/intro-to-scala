...
object OptionExercises2 {
  ...
  def findJobDescriptionGivenJobIdOrElse2(jobId: JobId): String = {
    val maybeDesc: Option[String] = findJobDescriptionGivenJobId1(jobId)
    maybeDesc.getOrElse(s"Job with id $jobId does not exist")
  }
  ...
  def findJobIdByHumanId(humanId: HumanId): Option[JobId] = {
    val maybeHuman: Option[Human] = findHumanById(humanId) // Some(Human(name: String, maybeJobId: Option[JobId]))
  ...
  def findJobByHumanId(humanId: HumanId): Option[Job] = {
    val maybeJobId: Option[JobId] = findJobIdByHumanId(humanId) // Some(1)
  ...
  def findJobNameByHumanId(humanId: HumanId): Option[String] = {
    val maybeJob: Option[Job] = findJobByHumanId(humanId) // Some(Job("Teacher", "Expert in their field"))
